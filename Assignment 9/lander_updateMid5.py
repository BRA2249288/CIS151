...
class ProxyObjectOne(spriteEngine.SuperSprite):
    def __init__(self, scene):
        ...
        #self.y = platform.y
        #self.x = platform.x
        self.setPosition((self.scene.platform.x -25, self.scene.platform.y -25))
        self.scene.platform.bulletShoot()
class ProxyObjectTwo(spriteEngine.SuperSprite):
    def __init__(self, scene):
        spriteEngine.SuperSprite.__init__(self, scene)
        self.setImage("blank.gif")
        self.imageMaster = pygame.transform.scale(self.imageMaster, (25, 25))
        self.platform = Platform(self)
        #self.y = platform.y
        #self.x = platform.x
...
class Game(sceneEngine.Scene):
    def __init__(self):
        ...
        if level == 2:         
            ...
            self.sprites = [self.lander, self.bullet, self.barrierone, self.barriertwo, self.barrierthree, self.enemynpc, self.platform, self.lblInfo]
            #sets the default gravity variable
            self.gravity = .02
            #sets the default Wind variable
            self.wind = .02

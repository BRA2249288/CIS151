...
class Rock(spriteEngine.SuperSprite):
    ...
    def spit(self, rank=3):
     def __init__(self, position, create_asteroid_callback, rank=3):
        ...
        self.position = (self.ship.position.y, self.ship.position.x)
        ...
        scale = size_to_scale[rank]
        self.setImage("asteroid_vesta.jpg")
        self.imageMaster = \
            pygame.transform.scale(self.imageMaster, (0, rank))
        if self.rank > 1:
         for _ in range(2):
            rock = Rock(
                self.position, self.create_rock_callback, self.rank - 1
            )
            self.create_rock_callback(rock)
